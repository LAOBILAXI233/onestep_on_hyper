...
import com.hyper.sidebar.util.ResolveInfoGroup.SameGroupComparator;
...
public class ResolveInfoManager extends SQLiteOpenHelper {
    ...
    static {
        ...
        sBlackList.add("com.hyper.sidebar");
    ...
    private List<ResolveInfoGroup> getShareTargets(String mimeType, Boolean multiple) {
        ...
        String[] actions = multiple == null
                ? ACTIONS
                : new String[] { multiple.booleanValue()
        ...
        for (String action : actions) {
            ...
            try {
                ...
                LSPLogger.w("ResolveInfoManager: cannot query " + action
                        + " for " + mimeType, e);
                continue;
            }
            if (infos == null) continue;
            resolvedCount += infos.size();
            for (ResolveInfo info : infos) {
                if (info == null || info.activityInfo == null) continue;
                String packageName = info.activityInfo.packageName;
                if ("com.hyper.sidebar".equals(packageName)) {
                    continue;
                }
                ResolveInfoGroup group = byPackage.get(packageName);
                if (group == null) {
                    group = new ResolveInfoGroup(mContext);
        ...
        result.addAll(byPackage.values());
        LSPLogger.i("ResolveInfoManager: mime=" + mimeType
                + " multiple=" + (multiple == null ? "unknown" : multiple)
