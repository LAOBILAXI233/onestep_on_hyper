...
import com.hyper.sidebar.util.DataManager;
import com.hyper.sidebar.util.FileInfo;
import com.hyper.sidebar.util.ImageInfo;
import com.hyper.sidebar.util.RecentClipManager;
...
/**
 * Expandable glass surface shown over the compact media card.
 *
 * The view owns only lightweight previews. Full clipboard, image and file content remains
 * in ContentView and is opened through SidebarController when a section is selected.
 */
public class MediaGlassPanel extends FrameLayout {
    ...
    public MediaGlassPanel(Context context, AttributeSet attrs, int defStyleAttr,
        ...
        // A layout may start GONE while still keeping a fully expanded resting state.
    ...
    /**
     * Sets interactive expansion in the range [0, 1]. A caller should finish the gesture with
     * setExpanded() so the view can settle and update visibility.
     */
    ...
    /**
     * Applies blur to a dedicated snapshot/sibling layer behind this panel. Applying a
     * RenderEffect directly to the panel would blur its labels and controls as well.
     */
    ...
    public boolean onInterceptTouchEvent(MotionEvent event) {
        ...
        if (action == MotionEvent.ACTION_UP || action == MotionEvent.ACTION_CANCEL) {
            // Once a horizontal close gesture has been intercepted, keep returning true
            // until onTouchEvent settles it. This preserves the child's normal tap path.
    ...
    private void applyBackdropBlur() {
        ...
        // Pre-S hosts retain the translucent wash, rim and elevation as the visual fallback.
    }
