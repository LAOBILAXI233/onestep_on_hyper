...
public class ClipboardAdapter extends BaseAdapter{
    ...
    private final ClipboardItemView.OnDeleteListener mDeleteListener =
            new ClipboardItemView.OnDeleteListener() {
        @Override
        public boolean onDelete(CopyHistoryItem item) {
            boolean removed = mClipManager.remove(item);
            updateDataList();
            return removed;
        }
    };
    ...
    public Object getItem(int position) {
        int offset = 0;
        for (int i = 0; i < Utils.Interval.DAY_INTERVAL.length; ++i) {
            int intervalCount = getIntervalCount(i);
            if (position < offset + intervalCount) {
                List<CopyHistoryItem> items = mIntervals.get(i);
                return items == null ? null : items.get(position - offset);
            }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if(interval_line == sNeedExpandNumber[our_interval] - 1 && !mExpand[our_interval] && interval_line != intervalInfos.size() - 1) {
            ...
            civ.showItem(intervalInfos.get(interval_line), mDeleteListener);
        }
        return civ;
    }
    ...
    class showMoreListener implements View.OnClickListener {
        ...
        public void onClick(View v) {
            mView.reset();
            mView.showItem(mItem, mDeleteListener);
